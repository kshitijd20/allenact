...
for metric in metrics:
    metric_directory = os.path.join(root_results_dir,"PointNaviThorPPOSimpleConvGRU")
...
ablation_results = {}
unit_types = ['random','agent','reachability']
...
for metric in metrics:    
    ...
    for unit_type in unit_types:
        ...
        for unit_removed in num_units_removed:
            metric_directory = os.path.join(root_results_dir,"PointNaviThorPPOSimpleConvGRU_" + unit_type + "_ablation_" + str(unit_removed))
            ...
            for metric_file in list_of_files[-1:]:
                metric_data = load_metric(metric_file)
                print(metric_file)
...
result_file = 'ablation_results_pointnav.json'
