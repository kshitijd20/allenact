...
class PointNavActorCriticSimpleConvRNNDummyTarget(ActorCriticModel[CategoricalDistr]):
    ...
    def get_target_coordinates_encoding(self, observations):
        dummy_observations = torch.full_like(observations[self.goal_sensor_uuid],10)
        
        observations[self.goal_sensor_uuid] = dummy_observations
        if self.embed_coordinates:
            return self.coordinate_embedding(
                observations[self.goal_sensor_uuid].to(torch.float32)
            )
...
class PointNavActorCriticSimpleConvRNNDummyImage(ActorCriticModel[CategoricalDistr]):
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: SpaceDict,
    ...
    def forward(  # type:ignore
        ...
        if not self.is_blind:
            ...
            if self.sensor_fusion:
                perception_embed = self.sensor_fuser(perception_embed)
            perception_embed = torch.full_like(perception_embed,0)
            x = [perception_embed] + x
...
class ResnetTensorPointNavActorCriticDummyImage(ActorCriticModel[CategoricalDistr]):
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: SpaceDict,
        ...
        if (
            ...
            self.goal_visual_encoder = ResnetTensorGoalEncoderDummyImage(
                self.observation_space,
                goal_sensor_uuid,
                resnet_preprocessor_uuid,
                goal_dims,
...
class ResnetTensorPointNavActorCriticDummyTarget(ActorCriticModel[CategoricalDistr]):
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: SpaceDict,
        ...
        if (
            ...
            self.goal_visual_encoder = ResnetTensorGoalEncoderDummyTarget(
                self.observation_space,
                goal_sensor_uuid,
                resnet_preprocessor_uuid,
                goal_dims,
...
class ResnetTensorGoalEncoderDummyTarget(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
    ...
    def forward(self, observations):
        ...
        #print("Observations are ", observations[self.goal_uuid])
        dummy_observations = torch.full_like(observations[self.goal_uuid],np.random.random_sample()*4)
        
        observations[self.goal_uuid] = dummy_observations
        #print("Observations are ", observations[self.goal_uuid])
        if self.blind:
            return self.embed_goal(observations[self.goal_uuid])
        embs = [
...
class ResnetTensorGoalEncoderDummyImage(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
    ...
    def forward(self, observations):
        ...
        #print("Observations are ", observations[self.goal_uuid])
        dummy_observations = torch.full_like(observations[self.resnet_uuid],0)
        
        observations[self.resnet_uuid] = dummy_observations
        #print("Observations are ", observations[self.goal_uuid])
        if self.blind:
            return self.embed_goal(observations[self.goal_uuid])
        embs = [
